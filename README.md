...
- 温度：进程内加载 LibreHardwareMonitor 读取 CPU Package DTS（见下方限制）
...
**网络**
- 总上下行速率
- 每进程网络流量排行（ETW 内核追踪 `Microsoft-Windows-Kernel-Network`，需管理员）
...
启动时自动请求 UAC 提权（用于 ETW 每进程网络流量与 LHM 温度驱动）；
拒绝提权也能运行，仅这两项降级为不可用。

## 构建 exe

本地一键构建（产物 `dist\SysMonitor.exe`）：
...
- **GitHub Actions**（[.github/workflows/build.yml](.github/workflows/build.yml)）：
  GitHub 托管 `windows-latest` runner，产物上传 artifact 并附加到 GitHub Release
...
`libs/` 下随附的第三方 .NET 库（LibreHardwareMonitor 等）各自遵循
MPL-2.0 / Apache-2.0 / MIT，详见 [libs/THIRD_PARTY.md](libs/THIRD_PARTY.md)。
